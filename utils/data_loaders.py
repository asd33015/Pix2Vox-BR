...
class ShapeNetDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        taxonomy_name, sample_name, rendering_images, volume,mask_Fimage, mask_Simage, gt_folder = self.get_datum(idx)

        if self.transforms:
            rendering_images = self.transforms(rendering_images)
        mask_Fimage = torch.from_numpy(mask_Fimage)
        mask_Simage = torch.from_numpy(mask_Simage)
        
        mask_Fimage = mask_Fimage.unsqueeze(0)
        mask_Simage = mask_Simage.unsqueeze(0)
        return taxonomy_name, sample_name, rendering_images, volume, mask_Fimage, mask_Simage, gt_folder
    ...
    def get_datum(self, idx):
        ...
        mask_Fpath = self.file_list[idx]['Fshape']
        mask_Spath = self.file_list[idx]['Sshape']
        gt_folder = volume_path.split('/')[-2]
        ...
        if self.dataset_type == DatasetType.TRAIN:
            ...
            selected_rendering_image_paths = [rendering_image_paths[i] for i in range(4)]
        ...
        for image_path in selected_rendering_image_paths:
            ...
            rendering_image = cv2.resize(rendering_image,(256,256))
        ...
        mask_image =  cv2.imread(mask_Fpath, cv2.IMREAD_UNCHANGED)
        mask_Fimage = cv2.cvtColor(mask_image, cv2.COLOR_BGR2GRAY).astype(np.float32) / 255.

        mask_image =  cv2.imread(mask_Spath, cv2.IMREAD_UNCHANGED)
        mask_Simage = cv2.cvtColor(mask_image, cv2.COLOR_BGR2GRAY).astype(np.float32) / 255.
        ...
        if suffix == '.mat':
            ...
            volume = f['data'][:]
            # volume = volume.data.astype(np.float32)
        return taxonomy_name, sample_name, np.asarray(rendering_images), volume,mask_Fimage, mask_Simage, gt_folder
...
class ShapeNetDataLoader:
    def __init__(self, cfg):
        ...
        self.mask_Spath_template = cfg.DATASETS.SHAPENET.MASK_SPATH
        self.mask_Fpath_template = cfg.DATASETS.SHAPENET.MASK_FPATH
    ...
    def get_files_of_taxonomy(self, taxonomy_folder_name, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            # volume_folder = os.path.dirname(volume_file_path)
            # total_views = len(os.listdir(volume_folder))
            # # view = random.randint(1,4)
            # sub = volume_folder.split('/')[-1]
            # name = '01_{}'.format(sub)
            volume_file_path = self.volume_path_template % (taxonomy_folder_name, sample_name)
            # print(volume_file_path)
            mask_Sfile_path =  self.mask_Spath_template % (taxonomy_folder_name, sample_name)
            mask_Ffile_path =  self.mask_Fpath_template % (taxonomy_folder_name, sample_name)
            if not os.path.exists(volume_file_path):
                ...
                # print(volume_file_path)                                                                       
                continue
            if not os.path.exists(mask_Sfile_path):
                print('[WARN] %s Ignore sample %s/%s since volume file not exists.' % (dt.now(), taxonomy_folder_name,
                                                                                  sample_name))
            
            if not os.path.exists(mask_Ffile_path):
                print('[WARN] %s Ignore sample %s/%s since volume file not exists.' % (dt.now(), taxonomy_folder_name,
                                                                                  sample_name))
                # print(volume_file_path)                                                                       
                continue
            ...
            for image_name in os.listdir(img_folder):
                ...
                if image_name.split('.')[-1] == 'txt':
                    continue
                img_file_path = self.rendering_image_path_template % (taxonomy_folder_name, sample_name, image_name)
            ...
            files_of_taxonomy.append({
                ...
                'Sshape': mask_Sfile_path,
                'Fshape': mask_Ffile_path,
            })
